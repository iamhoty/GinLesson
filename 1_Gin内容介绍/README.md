...
首先我们使用Goland创建一个Go项目，选择Go Modules
...
只要API程序**遵循了REST风格**，那就可以称其为RESTful API。目前在前后端分离的架构中，前后端基本都是通过RESTful API来进行交互。
...
Gin框架支持开发RESTful API的开发。**是一种规范和约定**
...
func StatCost() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 调用该请求的剩余处理程序 也就是先执行下一个函数 执行剩余的函数之后再往下执行
		c.Next()
		// 不调用该请求的剩余处理程序 但会执行完 当前的函数
		// c.Abort()
		// 计算耗时

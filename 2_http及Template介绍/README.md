...
Go语言模板引擎的使用可以分为三部分：**定义模板文件**、**解析模板文件**和**模板渲染**。
...
func sayHello(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("wd:", wd + "/hello.tmpl")
	// 解析指定文件生成模板对象
	tmpl, err := template.ParseFiles( wd + "/hello.tmpl")
...
func f1(w http.ResponseWriter, r *http.Request) {
	...
	// 告诉模板引擎，我现在多了一个自定义的函数kua 要在解析模板之前注册函数
	tmpl.Funcs(template.FuncMap{
		"kua": kua,
	})

	// 解析模板
	_, err = tmpl.ParseFiles(wd + "/hello.tmpl")

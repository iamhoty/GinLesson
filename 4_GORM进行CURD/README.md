...
Offset，指定开始返回记录前要跳过的记录数。**要和Limit一起使用才会生效**

```go
db.Debug().Limit(3).Offset(2).Find(&users)
//// SELECT * FROM `user2`   LIMIT 3 OFFSET 2;
...
db.Debug().Select("name, age").Find(&users).Scan(&results)
db.Debug().Table("user2").Select("name, age").Scan(&results)
...
如果你只希望更新指定字段，可以使用`Update`或者`Updates`（更新多个字段）
...
**警告** 删除记录时，请确保主键字段有值，GORM 会通过主键去删除记录，如**果主键为空，GORM 会删除该 model 的所有记录**。
